...
CMR_ERROR CMRlonglistmatAlloc(CMR* cmr, size_t memRows, size_t memColumns, size_t memNonzeros, LongListMat** presult)
{
  assert(cmr);
  assert(presult);

  CMR_CALL( CMRallocBlock(cmr, presult) );
  LongListMat* result = *presult;
...
CMR_ERROR CMRlonglistmatFree(CMR* cmr, LongListMat** plistmatrix)
{
  assert(cmr);
  assert(plistmatrix);

  LongListMat* listmatrix = *plistmatrix;
  if (!listmatrix)
    return CMR_OKAY;
...
CMR_ERROR CMRlonglistmatInitializeZero(CMR* cmr, LongListMat* listmatrix, size_t numRows, size_t numColumns)
{
  assert(cmr);
  assert(listmatrix);
...
CMR_ERROR CMRchrlistmatInitializeFromChrMatrix(CMR* cmr, ChrListMat* listmatrix, CMR_CHRMAT* matrix)
{
  assert(cmr);
  assert(listmatrix);
  assert(matrix);
...
CMR_ERROR CMRlonglistmatInitializeFromIntMatrix(CMR* cmr, LongListMat* listmatrix, CMR_INTMAT* matrix)
{
  assert(cmr);
  assert(listmatrix);
  assert(matrix);
  ...
  /* Initialze the zero matrix. */
  CMR_CALL( CMRlonglistmatInitializeZero(cmr, listmatrix, matrix->numRows, matrix->numColumns) );

  /* Fill nonzero data. */
  LongListMatNonzero* nonzero = &listmatrix->nonzeros[0];
  for (size_t row = 0; row < matrix->numRows; ++row)
  {
    size_t first = matrix->rowSlice[row];
    size_t beyond = matrix->rowSlice[row + 1];
  ...
  for (size_t i = 0; i < matrix->numNonzeros; ++i)
  {
    LongListMatNonzero* nz = &listmatrix->nonzeros[i];
...
CMR_ERROR CMRlonglistmatInitializeFromDoubleMatrix(CMR* cmr, LongListMat* listmatrix, CMR_DBLMAT* matrix, double epsilon)
{
  assert(cmr);
  assert(listmatrix);
  assert(matrix);
...
CMR_ERROR CMRchrlistmatInitializeFromChrSubmatrix(CMR* cmr, ChrListMat* listmatrix, CMR_CHRMAT* matrix,
...
CMR_ERROR CMRlonglistmatInitializeFromIntSubmatrix(CMR* cmr, LongListMat* listmatrix, CMR_INTMAT* matrix,
  CMR_SUBMAT* submatrix)
{
  assert(cmr);
  assert(listmatrix);
...
CMR_ERROR CMRchrlistmatInitializeFromChrSubmatrixComplement(CMR* cmr, ChrListMat* listmatrix, CMR_CHRMAT* matrix,
  CMR_SUBMAT* submatrix)
{
  assert(cmr);
  assert(listmatrix);
...
CMR_ERROR CMRlonglistmatInitializeFromIntSubmatrixComplement(CMR* cmr, LongListMat* listmatrix, CMR_INTMAT* matrix,
  CMR_SUBMAT* submatrix)
{
  assert(cmr);
  assert(listmatrix);
...
CMR_ERROR CMRlonglistmatPrintDense(CMR* cmr, LongListMat* listmatrix, FILE* stream)
{
  assert(cmr);
  assert(listmatrix);
  assert(stream);
  ...
  for (size_t row = 0; row < listmatrix->numRows; ++row)
  {
    LongListMatNonzero* nz = listmatrix->rowElements[row].head.right;
    for (size_t column = 0; column < listmatrix->numColumns; ++column)
    {
      if (column < nz->column)
        fprintf(stream, " 0");
      ...
      {
        assert(column == nz->column);
        fprintf(stream, " %lld", (long long) nz->value);
        nz = nz->right;
      }
      fflush(stream);
    }
