...
CMR_ERROR CMRchrmatPrintDense(CMR* cmr, FILE* stream, CMR_CHRMAT* matrix, char zeroChar, bool header)
{
  assert(cmr);
  assert(stream);
  assert(matrix);
  ...
  CMR_CALL( CMRallocStackArray(cmr, &rowEntries, matrix->numColumns) );
  ...
  CMR_CALL( CMRfreeStackArray(cmr, &rowEntries) );
...
bool CMRisBinaryDbl(CMR* cmr, CMR_DBLMAT* sparse, double epsilon, CMR_SUBMAT** psubmatrix)
{
  assert(sparse != NULL);
  ...
  for (int row = 0; row < sparse->numRows; ++row)
  {
    ...
    for (int entry = begin; entry < end; ++entry)
    {
      ...
      {
        if (psubmatrix)
          CMR_CALL( CMRsubmatCreate1x1(cmr, row, sparse->entryColumns[entry], psubmatrix) );
        return false;
      }
    }
  }
...
bool CMRisBinaryInt(CMR* cmr, CMR_INTMAT* sparse, CMR_SUBMAT** psubmatrix)
{
  assert(sparse != NULL);
...
bool CMRisBinaryChr(CMR* cmr, CMR_CHRMAT* sparse, CMR_SUBMAT** psubmatrix)
{
  assert(sparse != NULL);
...
bool CMRisTernaryDbl(CMR* cmr, CMR_DBLMAT* sparse, double epsilon, CMR_SUBMAT** submatrix)
{
  ...
  for (int row = 0; row < sparse->numRows; ++row)
  {
    ...
    for (int entry = begin; entry < end; ++entry)
    {
      ...
      {
        if (submatrix)
          CMR_CALL( CMRsubmatCreate1x1(cmr, row, sparse->entryColumns[entry], submatrix) );
        return false;
      }
    }
  }
...
bool CMRisTernaryInt(CMR* cmr, CMR_INTMAT* sparse, CMR_SUBMAT** psubmatrix)
{
  assert(sparse != NULL);
...
bool CMRisTernaryChr(CMR* cmr, CMR_CHRMAT* sparse, CMR_SUBMAT** psubmatrix)
{
  assert(sparse != NULL);
...
CMR_ERROR CMRchrmatFilter(CMR* cmr, CMR_CHRMAT* matrix, size_t numRows, size_t* rows, size_t numColumns, size_t* columns,
  CMR_CHRMAT** presult)
{
  assert(cmr);
  assert(matrix);
...
CMR_ERROR CMRchrmatFilterSubmat(CMR* cmr, CMR_CHRMAT* matrix, CMR_SUBMAT* submatrix, CMR_CHRMAT** presult)
{
  ...
  CMR_CALL( CMRchrmatFilter(cmr, matrix, submatrix->numRows, submatrix->rows, submatrix->numColumns, submatrix->columns,
    presult) );
